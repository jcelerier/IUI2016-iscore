...
\begin{abstract}
  The development of interactive shows and interactive user interfaces for exhibitions
...
while more interactive applications, for instance in museums, are generally written 
in domain-specific programming environments, like Max/MSP, Processing or OpenFrameworks.
  In this paper, we argue about the peculiar problematics that arise in such environments, and we present 
...
This is done in a single cohesive graphical user interface, built upon a few simple and novel primitives.
  i-score allows the creation of software meant to operate in a large parameter space, 
and enables artists to express easily both temporal logic and structured programming, 
with facilities for automating and mapping single and multi-dimensional parameters.
...
\subsubsection{Document models} % bof ici
...
Max, Processing, OF, PureData, React.[...], Integra Live, Unity \& envs de jeu, etc. (revoir slides), Chronic (cf. téléchargements), OpenMusic, Antescofo (et Ascograph), logiciels de la conférence sur appli réactives (cf. slack).


%[[ Examples de cas d'utilisation ]] (cas renaud, cas BY)

% [[ État de l'art et problèmes avec l'existant ]]

%% Interfaces pour création de contenu (flash, etc.)

%% Modèles de paramètres (parler de CORBA, dbus, DOM JS, DOM Qt... ?)

%% Modèles de contrôle dans le temps / dans la logique (du tout temporel au tout logique)
%% Parler des systèmes réactifs comme Max?
%% Différences avec Integra live (qui est plutôt orienté son)
...
Building from it : 
- Loops (that works like playheads)
...
Idée : transformations appliquées à une boîte
%% Synchro d'état, problématique du temps 
%% considéré comme instantané et relié "vu d'assez loin" 
%% et qui a besoin d'être séparé en mode "avant / après" .

%% Parler des multiples vues qu'on peut avoir

%% Mapping dans le temps et l'espace

%% Possibilités de contrôle par passage de messages

%% Est analogue à un petit système d'exploitation adapté pour applications multimédia



\section{Shortcomings (et pistes)}
%% Problématiques de trace d'exécution
%% Problématiques de débug et pistes envisagées
%% Possibilités pour boucles
%% Visualisation en temps réel des applications - faut-il faire un environnement entièrement intégré ? Étendre pour certains types en particulier ?
\section{Evaluation}
%% Amélioration en temps de développement pour interfaces artistiques
